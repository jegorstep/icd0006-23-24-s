...
const fetchPublication = async () => {
  ...
  if (response.data) {
    ...
    header.value = response.data.publicationHeader;
    selectedGame.value = response.data.boardGameId;
    description.value = response.data.description;
    location.value = response.data.gameLocation;
    playerAmount.value = response.data.playersAmount;
    gameDate.value = (new Date(response.data.gameDate)).toISOString().slice(0, 16);
  }
}

const  validateAndPublish = async () => {
  const response = await PublicationService.put(
      authStore.jwt, publication.value.id,
      {
...
<template>
  <div class="create-row">
    <div>
      <h2 class="main-header">Update Publication</h2>
    </div>
    <div>
    </div>
